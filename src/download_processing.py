...
class Processing:
    ...
    def download_space(self, rec_space_url: str, creator_id: str | int) -> str:
        ...
        if space.filename == None or space.filename == "":
            space.filename = f"{todays_date}"

        filename = (
            f"{space.filename}.m4a".encode("ascii", "ignore")
            .decode()
            .replace('"', "")
            .replace("'", "")
        ...
        if os.path.exists(os.path.join(self.DOWNLOADS_DIR, new_filename)):
            ...
            if os.path.exists(
                os.path.join(self.FINAL_DIR, f"{new_filename.replace('.m4a', '.mp3')}")
            ):
            ...
            shutil.move(
                os.path.join(self.CURRENT_DIR, f"{space.filename}.m4a"),
                os.path.join(self.DOWNLOADS_DIR, new_filename),
            )  # could do this in the volume change thing
    ...
    def remove_0_volume_from_file(self, filename: str, creator_id: str | int) -> dict:
        ...
        new_file_location = os.path.join(
            self.FINAL_DIR, year, month, str(creator_id), updated_mp3_filename
        )  # /final/2022/10/1234567890/FILE
        ...
        if os.path.exists(new_file_location):
            print(
                f"File {filename} already exists in final folder (has been edited already) - {new_file_location}."
            )
            return new_file_location
        ...
        data[str(creator_id)][
            updated_mp3_filename
        ] = f"/{year}/{month}/{creator_id}/{updated_mp3_filename}"
        ...
        os.system(
            f"bash ffmpeg_merge.sh '{filename}'"
        )  # this also converts to 96k bitrate
